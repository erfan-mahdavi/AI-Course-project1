...
class GridGraphics:
    def __init__(self, master, grid, cell_size=60):
        ...
        self.colors = {"background": "#f0f0f0", "grid_line": "#cccccc", "normal_cell": "#ffffff", "treasure_cell": "#90ee90", "thief_cell": "#ffcccb", "negative_cell": "#d3d3d3", "path": "#4682b4", "current_pos": "#ff7f50", "visited": "#e6e6fa", "start": "#32cd32", "goal": "#ff4500"}
        self.frame = ttk.Frame(master)
        self.frame.pack(padx=20, pady=20, fill="both", expand=True)
        canvas_width = self.n * cell_size + 1
        canvas_height = self.n * cell_size + 1
        self.canvas = tk.Canvas(self.frame, bg=self.colors["background"], width=min(canvas_width,800), height=min(canvas_height,600))
        ...
        self.canvas.configure(xscrollcommand=self.h_scroll.set, yscrollcommand=self.v_scroll.set, scrollregion=(0,0,canvas_width,canvas_height))
    ...
    def draw_grid(self):
        for i in range(self.n + 1):
            self.canvas.create_line(0, i * self.cell_size, self.n * self.cell_size, i * self.cell_size, fill=self.colors["grid_line"], width=1)
            self.canvas.create_line(i * self.cell_size, 0, i * self.cell_size, self.n * self.cell_size, fill=self.colors["grid_line"], width=1)
    ...
    def fill_cell(self, i, j, value, highlight=None):
        ...
        if hasattr(self, 'cell_ids') and cell_id in self.cell_ids:
            ...
            if not hasattr(self, 'cell_ids'):
                self.cell_ids = {}
            self.cell_ids[cell_id] = self.canvas.create_rectangle(x1, y1, x2, y2, fill=cell_color, outline=self.colors["grid_line"])
